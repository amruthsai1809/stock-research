    ...
    <div className="intelligence-grid">
      ...
      <div className="intelligence-detail">
        ...
        <section className="panel signal-tape">
          ...
          <div className="signal-columns"><div><b>Corporate insiders</b><strong>{signal?.insider.transactions.length ?? 0}</strong><span>open-market transaction lines from recent Forms 4/4-A</span>{signal?.insider.transactions.slice(0, 3).map((trade) => <a key={`${trade.accession}-${trade.ownerName}-${trade.transactionDate}-${trade.shares}`} href={trade.sourceUrl} target="_blank" rel="noreferrer"><Tag tone={trade.action === "purchase" ? "good" : "warn"}>{trade.action}</Tag><span>{trade.ownerName}<small>{trade.ownerRole} · {shortDate(trade.transactionDate)}</small></span><b>{trade.value == null ? `${trade.shares.toLocaleString()} sh.` : compactMoney(trade.value)}</b></a>)}</div><div><b>Tracked 13F managers</b><strong>{signal?.institutional.managersHolding ?? 0}</strong><span>active managers currently reporting the security</span><dl><div><dt>New</dt><dd className="positive">+{signal?.institutional.managersNew ?? 0}</dd></div><div><dt>Increased</dt><dd className="positive">+{signal?.institutional.managersIncreased ?? 0}</dd></div><div><dt>Reduced</dt><dd className="negative">−{signal?.institutional.managersReduced ?? 0}</dd></div><div><dt>Exited</dt><dd className="negative">−{signal?.institutional.managersExited ?? 0}</dd></div></dl><small>Portfolio period {shortDate(signal?.institutional.reportDate ?? null)} · filings can lag 45 days</small></div></div>
        </section>
