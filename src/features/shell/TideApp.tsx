...
export function TideApp() {
  ...
  if (!dataset) {
    return <ProductLoading failed={dataError} />;
  }

  return (
    <div className="app-shell">
      <aside className="sidebar">
        <button className="brand" onClick={() => setView("discover")} aria-label="TIDE home"><span className="brand__mark">T</span><span><b>TIDE</b><small>Equity research</small></span></button>
        <nav className="primary-nav" aria-label="Primary navigation">
          <span className="nav-label">Research</span>
          {navigation.map((item) => <button key={item.id} className={view === item.id || (item.id === "discover" && view === "company") ? "is-active" : ""} onClick={() => { setView(item.id); window.scrollTo({ top: 0, behavior: "smooth" }); }}><span>{item.glyph}</span>{item.label}{item.id === "dips" && <em>{stocks.filter((stock) => stock.drawdown52Week < -15).length}</em>}</button>)}
        </nav>
        ...
        <div className="sidebar-card">
          <span className="sidebar-card__icon">◎</span>
          <b>Local by design</b>
          <p>Your watchlist never leaves this browser.</p>
          <button onClick={() => setWatchlistOpen(true)}>Open workspace →</button>
        </div>
        <footer className="sidebar-footer"><span className="source-status"><i /> Sources healthy</span><small>SEC filings · EOD prices</small></footer>
      </aside>

      <div className="app-main">
        <header className="topbar">
          <button className="mobile-brand" onClick={() => setView("discover")}><span className="brand__mark">T</span>TIDE</button>
          ...
          <div className="topbar__actions">
            ...
            <button className="watchlist-trigger" onClick={() => setWatchlistOpen(true)}>★ <span>{watchlist.length}</span></button>
          </div>
        </header>

        <div className="data-banner"><span>Research snapshot</span><p>Prices are end-of-day and fundamentals come from SEC filings. TIDE is for research, not investment advice.</p><button onClick={() => setView("filings")}>View methodology →</button></div>

        <main className="content">
          {view === "discover" && <Discover stocks={stocks} onSelect={selectStock} onOpenDipFinder={() => setView("dips")} />}
          {view === "dips" && <DipFinder stocks={stocks} onSelect={selectStock} watchlist={watchlist} onToggleWatchlist={toggleWatchlist} />}
          {view === "company" && selectedStock && <CompanyResearch stock={selectedStock} isWatched={watchlist.includes(selectedStock.symbol)} onToggleWatchlist={toggleWatchlist} onOpenValuation={() => setView("valuation")} />}
      ...
      <nav className="mobile-nav" aria-label="Mobile navigation">{navigation.map((item) => <button key={item.id} className={view === item.id ? "is-active" : ""} onClick={() => setView(item.id)}><span>{item.glyph}</span><small>{item.label}</small></button>)}</nav>
...
function downloadWatchlist(symbols: string[]) {
  const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), symbols }, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = "tide-watchlist.json";
  anchor.click();
...
function readWatchlist(): string[] {
  ...
  try {
    const saved: unknown = JSON.parse(localStorage.getItem("tide-watchlist") || "[]");
...
function readTheme(): "light" | "dark" {
  ...
  return localStorage.getItem("tide-theme") === "dark" ? "dark" : "light";
}
