...
export function Screener({ stocks, onSelect }: { stocks: AnalyzedStock[]; onSelect: (symbol: string) => void }) {
  ...
  return (
    <div className="view-stack">
      ...
      <section className="panel screener-builder">
        <div className="panel-heading"><div><span className="eyebrow">Screen logic</span><h2>All conditions must pass</h2></div><button className="text-button" onClick={() => { setMinimumQuality(50); setMaximumDrawdown(0); setPositiveFcf(true); setSector("All sectors"); }}>Reset filters</button></div>
        <div className="filter-builder-grid">
          <label className="range-field"><span><b>Quality score</b><strong>≥ {minimumQuality}</strong></span><input type="range" min="20" max="90" value={minimumQuality} onChange={(event) => setMinimumQuality(Number(event.target.value))} /></label>
          <label className="select-field select-field--large"><span>52W drawdown</span><select value={maximumDrawdown} onChange={(event) => setMaximumDrawdown(Number(event.target.value))}><option value="0">Any drawdown</option><option value="-10">Down 10%+</option><option value="-20">Down 20%+</option><option value="-30">Down 30%+</option></select></label>
          <label className="select-field select-field--large"><span>Sector</span><select value={sector} onChange={(event) => setSector(event.target.value)}>{sectors.map((item) => <option key={item}>{item}</option>)}</select></label>
          <label className="toggle-field"><span><b>Positive free cash flow</b><small>Latest fiscal year</small></span><input type="checkbox" checked={positiveFcf} onChange={(event) => setPositiveFcf(event.target.checked)} /></label>
        </div>
      </section>
...
function downloadCsv(stocks: AnalyzedStock[]) {
  ...
  anchor.href = url;
  anchor.download = "tide-screen.csv";
  anchor.click();
