...
export function DipFinder({ stocks, onSelect, watchlist, onToggleWatchlist }: { stocks: AnalyzedStock[]; onSelect: (symbol: string) => void; watchlist: string[]; onToggleWatchlist: (symbol: string) => void }) {
  ...
  return (
    <div className="view-stack">
      <header className="dip-workbench-header">
        <div><span className="hero-panel__kicker"><span className="live-dot" /> TIDE signal engine</span><h1>Dip Finder</h1><p>Separate ordinary price pain from financially stronger pullbacks. Select any row to inspect the evidence.</p></div>
      ...
      <section className="filter-ribbon" aria-label="Dip Finder filters">
        <div className="segmented-control">
          {presets.map((item) => <button key={item.id} className={preset === item.id ? "is-active" : ""} aria-pressed={preset === item.id} onClick={() => setPreset(item.id)}>{item.label}</button>)}
        </div>
        <div className="filter-ribbon__right">
          ...
          <div className="view-switch" aria-label="Results view"><button className={view === "rank" ? "is-active" : ""} aria-pressed={view === "rank"} onClick={() => setView("rank")}>Ranked</button><button className={view === "map" ? "is-active" : ""} aria-pressed={view === "map"} onClick={() => setView("map")}>Map</button></div>
        </div>
      </section>
      ...
      <div className="dip-layout">
          ...
          {view === "rank" ? (
            <div className="dip-table-wrap">
              <table className="data-table dip-table">
                ...
                <tbody>{filtered.map((stock, index) => (
        ...
        {selectedStock && (
          <aside className="panel signal-inspector">
            ...
            <InteractivePriceChart prices={selectedStock.prices} symbol={selectedStock.symbol} name={selectedStock.name} height={190} compact initialRange="6M" />
