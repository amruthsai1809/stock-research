...
The scheduled updater treats the current Cloudflare deployment as its last-known-good baseline. It loads stable archives and current-year deltas, requests only the newest market sessions, merges by trading date, trims to the ten-year contract, and carries forward SEC-derived annuals. Source-specific circuit breakers stop a shared GitHub runner from retrying the same SEC denial across thousands of companies. Institutional profiles reuse unchanged deployed quarters and fetch filing documents only when the submissions feed or official quarterly master index exposes a new accession. The original full-refresh path remains available for controlled backfills; routine CI refreshes do not depend on SEC accepting shared GitHub runner IP addresses.
